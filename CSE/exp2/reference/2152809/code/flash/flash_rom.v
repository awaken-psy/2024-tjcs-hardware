...
module flash_rom(
    input wire wb_clk_i, // Wishbone总线的时钟信号
    input wire wb_rst_i, // 复位信号，用于初始化或重置模块状态
    input wire wb_cyc_i, // 表示一个Wishbone总线周期的开始和结束
    input wire wb_stb_i, // 选通信号，表明Wishbone主设备发起了一个有效的传输
    input wire wb_we_i, // 写使能信号，区分读写操作
    input wire [3:0] wb_sel_i, // 字节选择信号，用于选择32位数据中的有效字节
    input wire [23:0] wb_adr_i, // 地址信号，指定访问flash存储的地址
    input wire [31:0] wb_dat_i, // 写数据信号，从Wishbone总线传输到flash存储的数据
    output reg [31:0] wb_dat_o, // 读数据信号，从flash传输到Wishbone总线的数据
    output reg        wb_ack_o, // 应答信号，表示数据传输完成
    
    input wire flash_continue, // 外部信号，控制模块的持续操作
    
    output reg cs_n, // 片选信号，控制flash存储设备的使能状态
    input  sdi, // 串行数据输入
    output reg sdo, // 串行数据输出
    output reg wp_n, // 写保护信号
    output reg hld_n // 保持信号
    );
