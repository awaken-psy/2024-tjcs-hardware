...
module Counter8(
    ...
    wire [2:0] Q;
    wire O_CLK;
    Divider #(100000000) dv1(.I_CLK(CLK),.rst(rst_n),.O_CLK(O_CLK));
    JK_FF JK0(.J(1'b1),.K(1'b1),.CLK(O_CLK),.RST_n(rst_n),.Q1(Q[0]),.Q2());
    JK_FF JK1(.J(Q[0]),.K(Q[0]),.CLK(O_CLK),.RST_n(rst_n),.Q1(Q[1]),.Q2());
    JK_FF JK2(.J(Q[0]&Q[1]),.K(Q[0]&Q[1]),.CLK(O_CLK),.RST_n(rst_n),.Q1(Q[2]),.Q2());
    assign oQ = Q ;
